...
export default class IComment extends mixins(JhiDataUtils) {
  ...
  public iComments: IIComment[] = [];

  public isFetching = false;

  public mounted(): void {
    this.retrieveAllIComments();
  }
  ...
  public retrieveAllIComments(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.iComments.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IIComment): void {
  ...
  public removeIComment(): void {
    this.iCommentService()
      .delete(this.removeId)
      .then(() => {
        const message = this.$t('blogNewApp.iComment.deleted', { param: this.removeId });
